...
export default function DashboardScreen() {
  ...
  const loadData = useCallback(
    async (silent = false, fetchChart = false) => {
  ...
  const candles30 = useMemo(
    () => chartData?.candles.slice(-30) ?? [],
    [chartData],
  );
      ...
      <View style={styles.balanceRow}>
        ...
        <View style={[styles.balanceCard, { backgroundColor: theme.card }]}>
          ...
          <Text
            style={[
              styles.balanceValue,
              { color: pnl >= 0 ? theme.success : theme.error },
            ]}
          >
            {pnl >= 0 ? "+" : ""}
            {formatCurrency(pnl)}
      ...
      <View style={[styles.chartCard, { backgroundColor: theme.card }]}>
        <View style={styles.chartHeader}>
          <View style={styles.chartAsset}>
            <Text style={[styles.chartAssetText, { color: theme.text }]}>
              {asset}
            </Text>
          ...
          <View style={styles.legendRow}>
            <View style={styles.legendItem}>
              <View
                style={[styles.legendDot, { backgroundColor: "#00d4aa" }]}
              />
            ...
            <View style={styles.legendItem}>
              ...
              />
              <Text style={[styles.legendText, { color: theme.textSecondary }]}>
                MA{activeStrategy?.ma_short_period ?? 9}
              </Text>
            </View>
            <View style={styles.legendItem}>
              <View
              ...
              />
              <Text style={[styles.legendText, { color: theme.textSecondary }]}>
                MA{activeStrategy?.ma_long_period ?? 21}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.volumeContainer}>
          {(candles30.length >= 2
            ? candles30
            : Array.from({ length: 30 }, (_, i) => ({ volume: 50 + i * 3 }))
          ).map((c, i) => {
            ...
            const isGreen =
              candles30.length >= 2
                ? (candles30[i]?.close ?? 0) >=
                  (candles30[i - 1]?.close ?? candles30[i]?.close ?? 0)
                : i % 3 !== 0;
            return (
              <View
                key={i}
